 ...
 * I modified the library so it can rely on the FlexWire library (Bernhard Nebel)
 *
...
{
    flex.beginTransmission(APDS9930_I2C_ADDR);
    flex.write(val);
    if( flex.endTransmission() != 0 ) {
        return false;
    }
...
bool APDS9930::wireReadDataByte(uint8_t reg, uint8_t &val)
{
    ...
    /* Read from register */
    flex.requestFrom(APDS9930_I2C_ADDR, 1);
    while (flex.available()) {
        val = flex.read();
    }
                                        ...
                                        unsigned int len)
{
    ...
    /* Read block data */
    flex.requestFrom(APDS9930_I2C_ADDR, len);
    while (flex.available()) {
        if (i >= len) {
            return -1;
        }
